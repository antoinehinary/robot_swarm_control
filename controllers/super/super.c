...
#define V_MAX           0.128   // Maximum speed of a robot
...
#define INIT_MESSAGE 0          // Initial message
#define WEIGHT_MESSAGE 2        // Weight message
...
#define SWARMSIZE       10          // Swarm size for PSO
...
#define MAX_ROB     5           // number of robots in the flock
...
void reset(void) {
    ...
    for (int i = 0; i < FLOCK_SIZE; i++) {
        ...
        fprintf(Reynold1, ",v_%d", i); 
        fprintf(Laplace,  ",v_%d", i); 
        fprintf(Reynold2, ",v_%d", i); 
    }
...
float calculate_distance() {
    ...
    // Calculate the center of mass (COM) of the flock
    for (int i = 0; i < FLOCK_SIZE; i++) {
    ...
    // compute the deviation of each robot's distance from the COM
    for (int i = 0; i < FLOCK_SIZE; i++) {
    ...
    // Normalize the result and apply the final formula
...
float calculate_velocity() {
    ...
    // Computing the COM at the current time step
    for (int i = 0; i < FLOCK_SIZE; i++) {
        com_x += loc[i][0];
        com_y += loc[i][1];
    }
    ...
    // Computing the COM at the previous time step
    for (int i = 0; i < FLOCK_SIZE; i++) {
    ...
    // Computing the velocity of the COM
    float vel_x = (com_x - prev_com_x) / DELTA_T;
    float vel_y = (com_y - prev_com_y) / DELTA_T; 

    // Computing the projection onto the migration direction
    float flock_migrx = migrx - com_x;
    float flock_migry = migry - com_y;
    float migration_magnitude = sqrtf(flock_migrx * flock_migrx + flock_migry * flock_migry);
...
float calculate_velocity_laplace() {
    ...
    for (int k = 0; k < FLOCK_SIZE; k++) {
        float vel_x = (loc[k][0] - prev_loc[k][0]) / DELTA_T; 
        float vel_y = (loc[k][1] - prev_loc[k][1]) / DELTA_T; 

        float vel_magnitude = sqrtf(vel_x * vel_x + vel_y * vel_y);
        v_t += vel_magnitude / D_MAX;
    }
 ...
 */
void log_metrics(int time_log) {

    // Compute the metrics based on the current state
    if (state == 0 || state == 2) { // Reynold mode
        o_t = calculate_orientation();
        d_t = calculate_distance();
        v_t = calculate_velocity(); 
        m_fl_t = o_t * d_t * v_t;  
    } else { // Laplace mode
        d_t = calculate_distance_laplace(); 
        v_t = calculate_velocity_laplace(); 
        m_fl_t = d_t * v_t;                
    }

    if (state == 1) {
        fprintf(Laplace, "%d,%f,%f,%f", time_log, d_t, v_t, m_fl_t);
        fprintf(csv_file, "%d,%f,%f,%f,%f", time_log, 0.0, d_t, v_t, m_fl_t);
        ...
        fprintf(Reynold1, "%d,%f,%f,%f,%f", time_log, o_t, d_t, v_t, m_fl_t);
        fprintf(csv_file, "%d,%f,%f,%f,%f", time_log, o_t, d_t, v_t, m_fl_t);
        for (int i = 0; i < FLOCK_SIZE; i++) {
        ...
        fprintf(csv_file, "%d,%f,%f,%f,%f", time_log, o_t, d_t, v_t, m_fl_t);
        fprintf(Reynold2, "%d,%f,%f,%f,%f", time_log, o_t, d_t, v_t, m_fl_t);
        // Append positions of all robots
        for (int i = 0; i < FLOCK_SIZE; i++) {
...
int main(int argc, char *args[]) {
    ...
    double bestfit=0.0, bestw[DATASIZE];    // Best fitness and best weights
