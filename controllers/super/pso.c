...
double* pso(int n_swarmsize, int n_nb, double lweight, double nbweight, double vmax, double min, double max, int iterations, int n_datasize, int n_robots) {
  ...
  double gbest[n_swarmsize][n_datasize];   // Current best neighborhood
  double gbestperf[n_swarmsize];           // Current best neighborhood performance
  ...
  wb_supervisor_set_label(0,label,0.01,0.01,0.1,0xffffff,0,FONT); //affiche le nombre d'iteration sur webots
  ...
  updateNBPerf(lbest,lbestperf,gbest,gbestperf,neighbors);  // Find best neighborhood performances
  ...
  for (k = 0; k < iterations; k++) {
    ...
    updateNBPerf(lbest,lbestperf,gbest,gbestperf,neighbors);
  ...
  for (i = 0; i < swarmsize; i+=robots) { //divide the swarm into groups of robots=1 here =5 pour Project
    for (j=0;j<robots && i+j<swarmsize;j++) {
      ...
      for (k=0;k<datasize;k++)
        particles[j][k] = swarm[i+j][k]; //current value in the kth dimension for the jth particle in the current group
    }
    // USER MUST IMPLEMENT FITNESS FUNCTION
    if (type == EVOLVE_AVG) { //fitness and averages it with the previous fitness value for each particle, weighted by the number of evaluations (age)
      ...
      for (j=0;j<robots && i+j<swarmsize;j++)
	       perf[i+j] = fit[j];
    } else if (type == SELECT) {   // fitness multiple times (5 iterations) for each particle and averages the results-> more accurate
      for (j=0;j<robots && i+j<swarmsize;j++)
	       perf[i+j] = 0.0;
      for (k=0;k<5;k++) {
	       fitness(particles,fit,neighbors);
...
void updateLocalPerf(double swarm[swarmsize][datasize], double perf[swarmsize], double lbest[swarmsize][datasize], double lbestperf[swarmsize], double lbestage[swarmsize]) {
  ...
  for (i = 0; i < swarmsize; i++) {
    if (perf[i] > lbestperf[i]) {
      ...
      lbestperf[i] = perf[i];
      lbestage[i] = 1.0; //resets age of the best solution
    }
  }
}
...
void updateNBPerf(double lbest[swarmsize][datasize], double lbestperf[swarmsize],
		      double gbest[swarmsize][datasize], double gbestperf[swarmsize],
		      int neighbors[swarmsize][swarmsize]) {
  ...
  for (i = 0; i < swarmsize; i++) {
    ...
    for (j = 0; j < swarmsize; j++) {
      ...
      // If current performance of particle better than previous best, update previous best
      if (lbestperf[j] > gbestperf[i]) {
      	copyParticle(gbest[i],lbest[j]);
      	gbestperf[i] = lbestperf[j];
      }
    }
  }
}
